...
def rotationVector(vec, tarDir):
	"""
	tarDir向量旋转vec的角度
	"""
	rad = math.atan2(vec.x , vec.z)
	x = tarDir.x * math.cos(rad)  - tarDir.z * math.sin(rad)
	z = tarDir.x * math.sin(rad)  + tarDir.z * math.cos(rad)
	return Math.Vector3( x, tarDir.y, z)
